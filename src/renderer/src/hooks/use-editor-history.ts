...
export const useEditorHistory = (editor: Editor | null) => {
    ...
    const [current, setCurrent] = useState<HistoryState>({
        lastAction: null,
        recentActions: [],
        canUndo: false,
        canRedo: false,
    ...
    const restoreHistoryAction = useCallback((actionId: string) => {
        const actionIndex = current.recentActions.findIndex(a => a.id === actionId);
        if (actionIndex === -1 || !editor) return;

        //const targetAction = current.recentActions[actionIndex];
        ...
        const newActions = current.recentActions.slice(0, actionIndex);
    ...
    }, [editor, current.recentActions]);

    const restoreToAction = useCallback((action: HistoryAction) => {
        const actionIndex = current.recentActions.findIndex(a => a.id === action?.id);
        if (actionIndex === -1 || !editor) return;
    ...
    return { current, restoreToAction, trackHistoryActions, restoreHistoryAction };
};