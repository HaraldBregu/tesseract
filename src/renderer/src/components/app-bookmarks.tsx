          ...
          {categories.map((category, index) => (
            <AppBookmarksCategoryContainer key={index}>
                  ...
                  <>
                    ...
                    >
                      <Button size="icon" variant="icon" className="h-6 w-6">
                        <Dropdown
                          className={`h-4 w-4 transition-transform ${expandedCategoryIds.includes(category.id) ? 'rotate-180' : ''}`}
                        />
                      </Button>
                      ...
                      <DropdownMenuContent align="end" className="w-30">
                        ...
                        <DropdownMenuItem
                          onClick={(e) => {
                            ...
                            onDeleteCategory(
                              category,
                              items.filter((data) => data.categoryId === category.id),
                              categories
                            )
                          }}
              ...
              {expandedCategoryIds.includes(category.id) &&
                items
                  .filter((data) => data.categoryId === category.id)
                  .map((item) => {
                    if (item.id === editingBookmarkId) {
                      return (
                        <BookmarkEditItem
...
const AppBookmarksContainer = forwardRef(
  ({ children, ...props }: AppBookmarksContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
...
const AppBookmarksHeaderContainer = forwardRef(
  ({ children, ...props }: AppBookmarksHeaderContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
      <>
        <div
          className="flex justify-between items-center p-2 sticky top-0 z-10"
          {...props}
          ref={ref}
        >
          {children}
...
const AppBookmarksBodyContainer = forwardRef(
  ({ children, ...props }: AppBookmarksBodyContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
      <>
...
const AppBookmarksCategoryContainer = forwardRef(
  (
    { children, ...props }: AppBookmarksCategoryContainerProps,
...
const AppBookmarksCategoryHeaderContainer = forwardRef(
  (
    { onClick, children, ...props }: AppBookmarksCategoryHeaderContainerProps,
    ref: ForwardedRef<HTMLDivElement>
  ) => {
    return (
      <>
...
const BookmarkContentItem = forwardRef(
  ({ value, ...props }: BookmarkContentItemProps, ref: ForwardedRef<HTMLDivElement>) => {
    if (!value || value === '') return null

    return (
      <div
        className="mx-1 mt-2 pl-4 border-l-4 border-blue-700 cursor-pointer"
        {...props}
        ref={ref}
      >
        <TooltipProvider>
...
const BookmarkDescriptionItem = forwardRef(
  ({ value, onEdit }: BookmarkDescriptionItemProps, ref: ForwardedRef<HTMLDivElement>) => {
    ...
    return (
      <div
        className="mt-4 text-xm mb-1 text-gray-600 cursor-pointer"
        onClick={handleClick}
        ref={ref}
      >
        <TooltipProvider>
...
const BookmarkAuthorItem = forwardRef(
  ({ value, ...props }: BookmarkAuthorItemProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
...
const BookmarkUpdatedDateItem = forwardRef(
  ({ value, ...props }: BookmarkUpdatedDateItemProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
...
const BookmarkItemLayout = forwardRef(
  ({ children, onClick, ...props }: BookmarkItemLayoutProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
...
const BookmarkItemContainer = forwardRef(
  (
    { children, onClick, className, ...props }: BookmarkItemContainerProps,
    ref: ForwardedRef<HTMLDivElement>
  ) => {
    return (
      <BookmarkItemLayout ref={ref} onClick={onClick} {...props}>
        <div
          className={cn('flex flex-col hover:bg-[#E5E5E5] rounded-md mb-4', className)}
          {...props}
        >
          {children}
        </div>
...
const BookmarkItem = forwardRef(
  (
    ...
    return (
      <BookmarkItemContainer
        className={cn(selected && 'bg-[#E5E5E5]')}
        onClick={() => {
          onClickContent()
          handleClick()
        }}
      ...
      >
        <div className="flex justify-between items-center pl-0 cursor-pointer flex-1 min-w-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="icon" variant="icon" className="h-6 w-6 flex-shrink-0">
                <More className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-30">
              <DropdownMenuItem onClick={() => onEdit()}>{t('bookmarks.edit')}</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onDelete()} className="text-red-600">
                {t('bookmarks.delete')}
              </DropdownMenuItem>
              {categories.length > 0 && (
                <DropdownMenuSub>
                  <DropdownMenuSubTrigger>{t('bookmarks.move')}</DropdownMenuSubTrigger>
                  <DropdownMenuPortal>
                    <DropdownMenuSubContent className="max-h-[300px] overflow-y-auto">
                      ...
                      {categories
                        .filter((data) => data.id !== item.categoryId)
                        .map((category, index) => (
...
const BookmarkEditItem = forwardRef(
  ({ item, onDone }: BookmarkEditItemProps, _: ForwardedRef<HTMLFormElement>) => {
