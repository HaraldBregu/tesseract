...
type TemplateItemAs = { type: 'input'; onChange: (event: any) => void; value: string }
...
function TemplateItem({ id, value, icon = 'other', name, as }: TemplateItemProps) {
  ...
  const radioItemClasses = cn('group flex flex-col items-center p-[2px]', {
    'cursor-pointer': !disabled
  })

  const iconOverlayClasses = cn('group-data-[state=checked]:bg-primary-80 rounded p-1 pb-0', {
    'pb-0': !isInput,
    'pb-[1px]': isInput
  })

  return (
    <RadioGroup.Item key={id} value={value} className={radioItemClasses}>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="mt-2 w-[131px] -mx-[2px] truncate text-[0.9375rem] font-normal group-data-[state=checked]:font-bold px-1">
                  {name}
                </div>
              </TooltipTrigger>
