      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
        <div className="flex-1">
          <AppRadioGroup
            items={[
              {
                label: t('headerFooter.show.none'),
                value: HeaderDisplayMode.NONE.toString(),
                className: 'text-[0.8125em] font-[600]'
              },
              {
                label: t('headerFooter.show.firstPage'),
                value: HeaderDisplayMode.FIRST_PAGE_ONLY.toString(),
                className: 'text-[0.8125em] font-[600]'
              },
              // {
              ...
              // },
              {
                label: t('headerFooter.show.allSections'),
                value: HeaderDisplayMode.ALL_SECTIONS.toString(),
                className: 'text-[0.8125em] font-[600]'
              }
            ]}
            ...
            onValueChange={(value) => {
              ...
              setSettings({
                ...
                startFromPage:
                  newValue === HeaderDisplayMode.FROM_SPECIFIC_PAGE
                    ? settings.startFromPage || 1
                    : undefined,
          ...
          <AppRadioGroup
            ...
            items={[
              {
                label: (
                  <div className="flex flex-col gap-2">
                    <span className="text-[0.8125em] font-[600]">
                      {t('headerFooter.show.selectedSections')}
                    </span>
            ...
            onValueChange={(value) => {
              ...
              setSettings({
                ...
                sectionsToShow:
                  newValue === HeaderDisplayMode.SELECTED_SECTIONS
                    ? settings.sectionsToShow || [1, 2, 3, 4]
                    : undefined
              })
            }}
          />
  ...
  ({
    ...
    return (
      <div className="flex flex-col">
        <Typography component="h6" className="text-[0.8125em] font-[600] mb-2">
          {position.label}
        </Typography>
...
const PaginationSetup = ({ settings, setSettings }: PaginationSetupProps) => {
  ...
  const positionOptions = useMemo(
    () => [
      ...
      {
        label: t('headerFooter.position.options.pageNumber'),
        value: HeaderContentType.PAGE_NUMBER.toString()
      },
      { label: t('headerFooter.position.options.date'), value: HeaderContentType.DATE.toString() },
      {
        label: t('headerFooter.position.options.dateTime'),
        value: HeaderContentType.DATE_TIME.toString()
      },
      {
        label: t('headerFooter.position.options.author'),
        value: HeaderContentType.AUTHOR.toString()
      },
  ...
  const handleSelect = (
    positionKey: 'leftContent' | 'centerContent' | 'rightContent',
    optionValue: string
  ) => {
    setSettings({
      ...settings,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
          {positions.map((position) => (
            <PositionColumn
              ...
              value={
                settings[position.key as 'leftContent' | 'centerContent' | 'rightContent'] ??
                HeaderContentType.NONE
              }
              onChange={(value) =>
                handleSelect(
                  position.key as 'leftContent' | 'centerContent' | 'rightContent',
                  value
                )
              }
            />
