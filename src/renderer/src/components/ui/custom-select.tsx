...
const CustomSelect: React.FC<CustomSelectProps> = ({
  ...
  const defaultTriggerClassName =
    'pl-[4px] pr-[2px] hover:bg-primary hover:text-white active:bg-primary active:text-white focus-visible:bg-primary focus-visible:text-white py-0 h-auto border-none shadow-none focus:ring-0 focus:ring-offset-0'
        ...
        {items.map((item, index) => (
          <Fragment key={`${item.value}-${index}`}>
            {showSeparators && index > 0 && (
              <CustomSelectSeparator className="text-xs font-normal" />
            )}
            <SelectItem
