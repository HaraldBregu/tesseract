...
export default function TextField({
    ...
    const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isValidInput) {
            ...
            // Only apply min/max validation if we have a complete valid number
        ...
        // Removed the else block that was resetting the input value
    ...
    // Add keyboard support for number increment/decrement
    const handleNumberKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (onKeyDown) {
            onKeyDown(e);
        }
        if (type === "number" && !e.defaultPrevented && !disabled) {
            if (e.key === "ArrowUp") {
                e.preventDefault();
                incrementValue();
    ...
    return (
        <div className={cn("grid w-full items-center gap-1.5 bg-inherit", className)}>
            {label && (
                <Label htmlFor={id} className={cn(error && "text-destructive dark:text-destructive-70", "font-semibold text-[12px] pl-2")}>
