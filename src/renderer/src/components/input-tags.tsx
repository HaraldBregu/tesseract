...
const InputTags = React.forwardRef<HTMLInputElement, InputTagsProps>(
  ({ className, value, onChange, chipClassName, inputClassName, ...props }, ref) => {
...
console.log({chipClassName})
    ...
    return (
      ...
      >
        {value.map((item) => (
          <Badge key={item} className={chipClassName}>
            {item}
            <Button
              variant="icon"
              size="icon"
        ...
        <input
          className={cn(inputClassName, 'flex-1 outline-none placeholder:text-neutral-500 dark:placeholder:text-neutral-400')}
