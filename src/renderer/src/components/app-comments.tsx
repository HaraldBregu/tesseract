...
const AppCommentsContainer = forwardRef(
  ({ children, ...props }: AppCommentsContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
...
const AppCommentsHeaderContainer = forwardRef(
  ({ children, ...props }: AppCommentsHeaderContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
      <>
        <div
...
const AppCommentsBodyContainer = forwardRef(
  ({ children, ...props }: AppCommentsBodyContainerProps, ref: ForwardedRef<HTMLDivElement>) => {
    return (
      <>
...
const AppCommentsCategoryHeaderContainer = forwardRef(
  (
    { onClick, children, ...props }: AppCommentsCategoryHeaderContainerProps,
...
const CommentContentItem = forwardRef(
  ({ value, className, ...props }: CommentContentItemProps, ref: ForwardedRef<HTMLDivElement>) => {
    if (!value || value === '') return null

    return (
      <div
        className={cn(
          'mx-1 mt-2 pl-4 border-l-4 border-blue-700 cursor-pointer text-gray-600',
          className
        )}
        {...props}
        ref={ref}
...
const CommentDescriptionItem = forwardRef(
  (
    { value, className, ...props }: CommentDescriptionItemProps,
    ref: ForwardedRef<HTMLDivElement>
  ) => {
    if (!value || value === '') return null
    ...
    return (
      <div
        className={cn('mt-4 text-xm mb-1 text-gray-600 cursor-pointer', className)}
        {...props}
        ref={ref}
      >
        <TooltipProvider>
...
const CommentItemLayout = forwardRef(
  (
    { onClick, className, children, ...props }: CommentItemLayoutProps,
    ref: ForwardedRef<HTMLDivElement>
  ) => {
    return (
      <div
        ref={ref}
        className={cn('px-2 mt-2 mb-4 cursor-pointer', className)}
        {...props}
        onClick={onClick}
      >
        {children}
...
const CommentItemContainer = forwardRef(
  (
    { children, onClick, className, ...props }: CommentItemContainerProps,
    ref: ForwardedRef<HTMLDivElement>
  ) => {
    return (
      <CommentItemLayout ref={ref} onClick={onClick} {...props}>
        <div className={cn('flex flex-col rounded-md mb-4', className)}>{children}</div>
      </CommentItemLayout>
    )
  }
)
        ...
        <div className="flex justify-between items-center pl-0 cursor-pointer flex-1 min-w-0">
          <div className={cn('flex items-center gap-2 flex-1 min-w-0', selected && 'text-white')}>
            <Comment
              ...
              fillcolor={selected ? 'white' : null}
            />
            <span className={cn('text-sm font-medium truncate text-ellipsis')}>
              {item.author}
              <p className="text-xs">{format(new Date(item.updatedAt), 'dd/MM/yyyy HH:mm')}</p>
            </span>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
            ...
            <DropdownMenuContent align="end" className="w-30">
              <DropdownMenuItem onClick={() => onEdit()}>{t('comments.edit')}</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onDelete()} className="text-red-600">
                {t('comments.delete')}
              </DropdownMenuItem>
              {categories.length > 0 && (
                <DropdownMenuSub>
                  <DropdownMenuSubTrigger>{t('comments.move')}</DropdownMenuSubTrigger>
                  <DropdownMenuPortal>
...
const CommentEditItem = forwardRef(
  ({ item, onDone }: CommentEditItemProps, _: ForwardedRef<HTMLFormElement>) => {
    ...
    return (
      <CommentItemContainer className="hover:bg-transparent" onClick={() => {}}>
        <Form {...form}>
