...
const Apparatuses = forwardRef(({
    ...
    const siglumList = useMemo(() => state.siglumList.map((siglum) => ({
        label: siglum.siglum.value,
        value: siglum
    })), [state.siglumList])
    ...
    const handleAddNewApparatus = useCallback((type: ApparatusType, index: number) => {
    ...
    const handleChangeType = useCallback((id: string, type: ApparatusType) => {
    ...
    // @REFACTOR: Move this to a separate component
    ...
    const handleInsertSiglum = useCallback((siglum: Siglum) => {
        if (!editorRef) return;
        editorRef.insertNodeContentsFromContent(siglum.siglum.content)
        editorRef.unsetBold()
        editorRef.unsetItalic()
        editorRef.unsetUnderline()
        editorRef.unsetSuperscript()
        editorRef.unsetSubscript()
    }, [editorRef])
