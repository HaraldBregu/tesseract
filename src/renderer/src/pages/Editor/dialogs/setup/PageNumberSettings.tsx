...
interface PageNumberModalProps {
    ...
    onSave: (pageNumberSettings: PageNumberSettings) => void;
}
...
const PageNumberSettings = ({
    ...
    const [settings, setSettings] = useState<PageNumberSettings>({
        toc: { pageNumeration: '3', numberFormat: '1', startingPointValue: 1 },
        intro: { pageNumeration: '3', numberFormat: '1', startingPointValue: 1 },
        crt: { pageNumeration: '3', numberFormat: '1', startingPointValue: 1 },
        biblio: { pageNumeration: '3', numberFormat: '1', startingPointValue: 1 }
    });
        ...
        return <Select
            ...
            <SelectContent>
                {numberFormat.map((type, index) => (
                    <SelectItem
                        className="font-thin text-grey-10 dark:text-grey-90"
                        value={type.value}
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <AppRadioGroup
                            items={[
                                ...
                                { label: <div className="flex flex-row gap-2 items-center mb-4">{t('pageNumber.start')} {selectStarterPoint(tabType)}</div>, value: '3', className: "text-[0.8125em] font-[600]" },
    ...
    useEffect(() => {
        const getPageNumberSettings = async () => {
            const pageNumberSettings = await window.doc.getPageNumberSettings();
            setSettings(prev => ({
                ...prev,
                ...pageNumberSettings
            }));
    ...
    return <Modal
        ...
        actions={[
            <Button key="cancel" className="w-full sm:w-[6em]" size="mini" intent={"secondary"} variant={"tonal"} onClick={onClose}>{t('buttons.cancel')}</Button>,
