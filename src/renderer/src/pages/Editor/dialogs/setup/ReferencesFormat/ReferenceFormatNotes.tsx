...
import { ReferencesAction, setNotesNumberFormat, setNotesNumeration, setNotesSectionStyle } from "./ReferenceFormatAction";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useCallback } from "react";
import { NUMERATION_OPTIONS, SECTION_LEVELS } from "./ReferenceFormatConstants";
            ...
            <div className="flex flex-row gap-10">
                ...
                <div className="flex flex-col gap-2 min-w-[156px]">
                    <Label className="text-secondary-30 text-[13px] font-semibold leading-[15px] dark:text-foreground hover:cursor-pointer">
                        {t("referencesFormat.numberFormat")}
                    </Label>
                    <Select
...
const Notes: React.FC<NoteProps> = ({
    ...
    return (
        <div className="flex flex-col gap-6">
            {/* <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-3 rounded text-sm mr-4" role="alert">
                <span className="font-bold block mb-1">{t("referencesFormat.warningBadge.title")}</span>
                {t("referencesFormat.warningBadge.message")}
            </div> */}
            <NotesSection
