...
const CustomizeToolbarSection: React.FC<CustomizeToolbarSectionProps> = ({
  ...
  return (
    <div
      key={section}
      className={cn(
        'flex flex-wrap items-start gap-2 border border-grey-80 rounded-md p-2',
        className
      )}
    >
      {/* Section Buttons */}
      {buttons.map(({ enabled, label, icon: Icon, action, className = 'w-[88px]' }) => (
        <Button
          key={action}
          disabled={!enabled}
          className={cn(
            'text-grey-10 gap-2 flex flex-col p-1 border-none shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 h-auto',
            className
          )}
        ...
        >
          ...
          <Label className="break-words whitespace-normal font-normal leading-[18px]">
            {label}
          </Label>
        </Button>
      ))}
...
const CustomizeToolbarModal: React.FC<CustomizeToolbarModalProps> = ({
  existingToolbarItems,
  isOpen,
  onCancel,
  onSaveToolbarOptions
}) => {
  ...
  const toolbarData: any = [
    {
      ...
      buttons: [
        ...
        {
          enabled: true,
          label: t('toolbar.nonPrintingCharacters'),
          icon: NonPrintingCharact,
          action: 'nonprinting'
        }
      ]
    },
    {
      ...
      buttons: [
        ...
        {
          enabled: true,
          label: t('toolbar.readingType'),
          icon: ReadingType,
          action: 'readingType'
        },
        {
          enabled: true,
          label: t('toolbar.readingSeperator'),
          icon: ReadingSeperator,
          action: 'readingSeparator'
        }
      ]
    },
    {
  ...
  const layoutToolbarData = [
    {
      ...
      buttons: [
        ...
        {
          enabled: false,
          label: t('toolbar.increaseIndent'),
          icon: IndentIncrease,
          action: 'indent'
        },
        {
          enabled: false,
          label: t('toolbar.decreaseIndent'),
          icon: IndentDecrease,
          action: 'outdent'
        }
      ]
    ...
    {
      section: 'search',
      buttons: [{ enabled: false, label: t('toolbar.search'), icon: Search, action: 'find' }]
    },
    {
      section: 'print',
      buttons: [
    ...
    <Modal
      ...
      actions={[
        ...
        <Button
          ...
          size="mini"
          intent={'primary'}
          onClick={() => onSaveToolbarOptions(selectedToolbarItems)}
        >
          {t('buttons.done')}
        </Button>
      ]}
