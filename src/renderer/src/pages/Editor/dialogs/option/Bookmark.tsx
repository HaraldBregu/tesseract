...
interface BookmarkProps {
    open: boolean
    onCancel: () => void
    onSelectCategory: (category: BookmarkCategory) => void
    categories: BookmarkCategory[] | null
}
...
const Bookmark = ({
    ...
    const uncategorizedItem = useMemo(() => ({
        id: undefined,
        name: t("bookmarks.category.uncategorized", "##Uncategorized##"),
    } as BookmarkCategory), [])
    ...
    const selectCategory = useCallback((category: BookmarkCategory) => {
        ...
        <AppDraggableDialog open={open} onOpenChange={onCancel}>
            <AppDraggableDialogContent className={cn("max-w-xs")}>
                <AppDraggableDialogHeader>
                    <div className="flex justify-between items-center">
                        <AppDialogTitle className={cn("text-md font-bold flex-1")}>
                            {t("bookmarks.category.title", "##Select Category##")}
                        </AppDialogTitle>
                        <AppButton
                            variant="transparent"
                            size="icon-sm"
                ...
                <div className="p-2">
                    <AppInput
                        type="search"
                        placeholder={t("bookmarks.category.filterPlaceholder", "##Filter categories...##")}
                        value={filterText}
                ...
                <AppDraggableDialogFooter>
                    <AppButton
                        ...
                        variant="secondary"
                        onClick={onCancel}>
                        <span>{t("bookmarks.category.cancel", "##Cancel##")}</span>
                    </AppButton>
                    <AppButton
                        className="px-6"
                        type="submit"
                        ...
                        <span>{t("bookmarks.category.select", "##Select##")}</span>
                    </AppButton>
                </AppDraggableDialogFooter>
