...
const PageSetup = ({
  ...
  const sectionClass2 = cn('[&:not(:last-child)]:border-b', 'flex', 'flex-col', 'gap-2', 'p-2')
  const sectionClass3 = cn('[&:not(:last-child)]:border-b', 'flex', 'flex-col', 'gap-4', 'p-2')
  ...
  const [optSetup, setOptSetup] = useReducer((state, action) => {
    switch (action.type) {
      ...
      case 'TEMPLATE_TYPE':
        return {
          ...state,
          template_type: state.template_type === 'Community' ? 'Private' : 'Community'
        }
      default:
        return state
    }
  ...
  const orientationPageClass = (curOrientation) =>
    cn(
      ...
      {
        'border-primary border-2': curOrientation === optSetup.paperSize_orientation && !readonly
      },
      {
        'border-secondary border-2': curOrientation === optSetup.paperSize_orientation && readonly
      }
    )
            ...
            <div className={sectionClass}>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectGroup>
                    <SelectItem value="custom">{t('pageSetup.component.selectCustom')}</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
                ...
                <div className="flex flex-row gap-2">
                  <div className="flex flex-col items-center">
                    <TextField
                      ...
                      disabled={readonly}
                    />
                    <Typography component="label">{t('pageSetup.position.top')}</Typography>
                  </div>
                  <div className="flex flex-col items-center">
                    <TextField
                      id="margin-bottom"
                      ...
                      disabled={readonly}
                    />
                    <Typography component="label">{t('pageSetup.position.bottom')}</Typography>
                  </div>
                </div>
                <div className="flex flex-row gap-2">
                  <div className="flex flex-col items-center">
                    <TextField
                      ...
                      disabled={readonly}
                    />
                    <Typography component="label">{t('pageSetup.position.left')}</Typography>
                  </div>
                  <div className="flex flex-col items-center">
                    <TextField
                      id="margin-right"
                      ...
                      disabled={readonly}
                    />
                    <Typography component="label">{t('pageSetup.position.right')}</Typography>
                  </div>
                </div>
              </div>
            </div>
