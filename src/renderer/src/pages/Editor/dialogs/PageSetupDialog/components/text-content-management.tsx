    ...
    <div className="text-[13px] flex flex-col justify-between h-full">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3">
          <Typography component="label" className={cn('ml-1 font-normal text-[13px]')}>
          ...
          <SortableArea<string>
            ...
            readSorted={(result) =>
              setApparatus.replace(
                result
                  .map((id) => apparatus.find((x) => x.id === id))
                  .filter((x): x is TElement => x !== undefined)
              )
            }
          />
          ...
          {apparatus.length < MAX_APPARATUS_NUMBER + 1 && (
            <div>
              ...
              >
                ...
                <span style={{ color: `${textColor}` }} className="inline-flex flex-1 text-[13px]">
