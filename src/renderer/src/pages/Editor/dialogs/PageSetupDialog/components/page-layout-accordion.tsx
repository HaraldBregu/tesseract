...
const PageLayoutAccordion = ({
  title,
  children,
  v,
  accordionDisabled,
  dragHandler
}: PageLayoutAccordionProps) => {
  const accordionTriggerClass =
    'flex flex-row justify-between w-full px-2 py-0 bg-white  hover:no-underline'
  ...
  return (
    <AccordionItem value={v}>
      <AccordionTrigger disabled={accordionDisabled} className={accordionTriggerClass}>
        ...
        <Typogaphy component="div">{title}</Typogaphy>
      </AccordionTrigger>
