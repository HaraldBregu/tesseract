...
const bookmarkSlice = createSlice({
  ...
  reducers: {
    addBookmark(
      ...
      if (action.payload.categoryId) newBookmark.categoryId = action.payload.categoryId
    ...
    updateBookmarkList(state, action: PayloadAction<any[]>) {
      const bookmarks = action.payload.reduce((b: any[], a: any) => {
    ...
    moveBookmarkToCategoryId(
      state,
      action: PayloadAction<{ bookmark: Bookmark; categoryId?: string }>
    ) {
    ...
    deleteBookmarkCategory(state, action: PayloadAction<BookmarkCategory>) {
      state.bookmarkCategories = state.bookmarkCategories.filter(
        (category) => category.id !== action.payload.id
      )
      state.bookmarks = state.bookmarks.filter(
        (bookmark) => bookmark.categoryId !== action.payload.id
      )
    },
    updateBookmarkCategory(state, action: PayloadAction<BookmarkCategory>) {
      const category = state.bookmarkCategories.find(
        (category) => category.id === action.payload.id
      )
      if (category) {
    ...
    moveBookmarksFromCategory(
      ...
      const bookmarks = state.bookmarks.filter(
        (bookmark) => bookmark.categoryId === action.payload.categoryId
      )
    ...
    selectBookmarkWithId(state, action: PayloadAction<string | null>) {
      ...
      state.selectedBookmark =
        state.bookmarks.find((bookmark) => bookmark.id === action.payload) ?? null
    }
  }
})
