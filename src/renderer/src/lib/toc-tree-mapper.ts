...
const extractSectionsFromGlobalText = (
  ...
  const items = Array.isArray(input) ? input : Array.isArray(input.content) ? input.content! : []

  if (items.length === 0) return []

  // 2) trova inizio e fine
  const startIndex = items.findIndex(
    (item) => isDividerItem(item) && item.attrs.sectionType === sectionType
  )
  if (startIndex === -1) return []

  const endIndex = items.findIndex((_, idx) => idx > startIndex && isDividerItem(items[idx]))
...
const createTocTreeStructure = (
  ...
  const headings = jsonData.content.filter(
    (item) =>
      item.type === 'heading' &&
      item.content &&
      item.content.length > 0 &&
  ...
  const generateHeadingId = (level: number, parentId: string | null = null): string | undefined => {
    ...
    if (tocSettings) {
      switch (level) {
        case 1:
          formatType = tocSettings.level1Format || '1'
          break
        case 2:
          formatType = tocSettings.level2Format || '2'
          break
        case 3:
          formatType = tocSettings.level3Format || '3'
          break
        case 4:
          formatType = tocSettings.level4Format || '4'
          break
        case 5:
          formatType = tocSettings.level5Format || '5'
          break
        case 6:
          formatType = tocSettings.level6Format || '6'
          break
        default:
          formatType = '1'
      }
...
const createTiptapJSONStructure = (
  treeItems: TreeItem[],
  ...
  const convertTreeItemToTiptap = (item: TreeItem): any => {
    ...
    const targetPosition = CONTAINER_WIDTH * 0.9

    // Precise heading font width factor based on style
    const headingFontFactor = isBold
      ? isItalic
        ? 0.75
        : 0.7 // Bold or Bold+Italic (wider)
      : isItalic
        ? 0.6
        : 0.65 // Italic or Normal

    // Calculate exact width of the heading text with all factors considered
    const titleFullWidth =
      titleText.length * fontSize * headingFontFactor + indentationWidth + padding
