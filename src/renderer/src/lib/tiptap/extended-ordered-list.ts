...
import { orderedListTypeToStyle, orderedListStyleToType } from '@/lib/utils/list-type-mappers';

/**
 * Extended OrderedList with support for different list types
...
export const ExtendedOrderedList = OrderedList.extend({
    ...
    return {
      ...
      listType: {
        ...
        parseHTML: element => {
          ...
          // Map CSS back to type using utility
          return orderedListStyleToType[styleAttr] || '1';
        },
        renderHTML: attributes => {
          const type = attributes.listType || '1';

          // Map HTML type to CSS list-style-type using utility
          const listStyleType = orderedListTypeToStyle[type] || 'decimal';

          const styles = [`list-style-type: ${listStyleType} !important`];
          const dataAttrs: Record<string, string> = { type };
  ...
  addCommands() {
    return {
      ...this.parent?.(),
      updateOrderedListMarkerStyles: () => ({ commands, state }) => {
        const { selection } = state;
        const { $from } = selection;

        // Find the orderedList node
        for (let depth = $from.depth; depth >= 0; depth--) {
          ...
          if (node.type.name === 'orderedList') {
            ...
            return commands.updateAttributes('orderedList', markerAttrs);
          }
        }
        return false;
      },
